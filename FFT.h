...
{
  const auto result = FFT_impl(n_sample, [data_first](int n_sample, int log2_of_n_sanmple) {
...
void InverseFFT(int n_sample, InputIterator in_first_Re, InputIterator in_first_Im,
  ...
  const auto result = FFT_impl(n_sample, [in_first_Re, in_first_Im](int n_sample, int log2_of_n_sample) {
    std::vector<std::complex<double>> input(n_sample);
    for (int i = 0; i < n_sample; i++)
    {
      int bit_inv_i = binary_inversion(i, log2_of_n_sample);
      input[i] = {static_cast<double>(*(in_first_Re + bit_inv_i)), static_cast<double>(*(in_first_Im + bit_inv_i)) * (-1)};
    }
    return input;
  });
